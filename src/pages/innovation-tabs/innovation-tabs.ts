...
import { FirstCreaterPage } from '../first-creater/first-creater';
...
/**
 * Generated class for the InnovationTabsPage page.
 *
...
@Component({
  selector: 'page-innovation-tabs',
  templateUrl: 'innovation-tabs.html',
  providers: [ChangelanguageProvider]
})
export class InnovationTabsPage {

  tab1Root = FirstCreaterPage ;
  tab2Root = VideoPage;
  tab3Root = BvPage;
  changeicon : any = 'md-add';
  textMyan : any = ["စွန့်ဦးတီထွင်မှု","ဗီဒီယို","Brand Voice"];
  textEng : any = ["Innovation","Video","Brand Voice"];
  textData : any = [];
  font : any ;
  pageTitle: any;
  noticount:any =0;
  ...
  menuCount(){
    ...
    this.http.post(this.funct.ipaddress2+'serviceAppHistory/saveType',parameter).map(res => res.json()).subscribe(result => {
      ...
      if(result.state){
        console.log("Innovation success menuCount >>>>>>>>");
      }
      else if(!result.sessionState){
        this.cdata.sessionAlert();
      }
      else{
        console.log("Innovation unsuccess menuCount >>>>>>>>");
      }
    },
        error => {
  ...
  updateNotiCount(){
    this.sqlite.create({
      ...
      db.executeSql("UPDATE notiCount SET count = 0 WHERE type = 'innovation'",[]).then((data) => {
      console.log("update data successfully", data);
      this.events.publish('innovationCount', 0);
      }, (error) => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad InnovationTabsPage');
  }
