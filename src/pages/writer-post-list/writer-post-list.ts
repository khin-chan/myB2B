...
import { IonicPage, NavController, NavParams , AlertController , LoadingController , ToastController , Platform , Events} from 'ionic-angular';
...
/**
 * Generated class for the WriterPostListPage page.
 *
...
@Component({
  selector: 'page-writer-post-list',
  templateUrl: 'writer-post-list.html',
  providers: [ChangelanguageProvider]
})
export class WriterPostListPage {
  ...
  getList(start,infiniteScroll){    
    ...
    this.http.post(this.funct.ipaddress2+"serviceArticle/getArticleListByContentWriter?usersk="+this.registerData.syskey+"&userid="+this.passData+'&sessionKey='+this.registerData.sessionKey, parm).map(res => res.json()).subscribe(data => {
...
getError(error,status){
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad WriterPostListPage');
  }
