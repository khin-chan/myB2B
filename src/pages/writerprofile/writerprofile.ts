...
import { NavController, NavParams, Platform, ToastController, LoadingController, AlertController } from 'ionic-angular';
...
/**
 * Generated class for the WriterprofilePage page.
 *
...
@Component({
  selector: 'page-writerprofile',
  templateUrl: 'writerprofile.html',
  providers: [ChangelanguageProvider]
})
export class WriterprofilePage {

  writerImg: any;
  passData: any;
  font: any;
  textMyan: any = ["ပင်မစာမျက်နှာ", "ကြိုက်တယ်", "မှတ်ချက်", "ဝေမျှမည်", "ဒေတာ မရှိပါ။", "အကြောင်းအရာများ", "ဗီဒီယို", "တွေ့ဆုံမေးမြန်းခြင်း", "ကြည့်ရန်", "ခု", "အီးမေးလ်", "ဖုန်းနံပါတ်", "လိပ်စာ"];
  textEng: any = ["Home", "Like", "Comment", "Share", "No result found", "Article", "Videos", "Interview", "View", "Count", "Email", "Phone", "Address"];
  textData: any = [];
  registerData: any;
  menuList: any;
  pinmenuList: any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public http: Http,
    public platform: Platform, public sanitizer: DomSanitizer, public datepipe: DatePipe,
    public toastCtrl: ToastController, public changefont: ChangefontProvider, public logger: EventLoggerProvider,
    public funct: FunctProvider, public storage: Storage, public alert: AlertController, public fba: FirebaseAnalytics,
    ...
    this.passData = navParams.get('data');
    console.log("writerprofile passdata == " + JSON.stringify(this.passData));
  ...
  getList(start, infiniteScroll) {
    ...
    this.http.post(this.funct.ipaddress2 + "serviceArticle/getArticleListByContentWriter?usersk=" + this.registerData.syskey + "&userid=" + this.passData.t1 + '&sessionKey=' + this.registerData.sessionKey, parm).map(res => res.json()).subscribe(data => {
  ...
  viewProfile() {
    this.navCtrl.push(ViewPhotoMessagePage, {
      data: this.writerImg + '/' + this.passData.t16,
  ...
  getLike(data, j, k, status) {
    ...
    console.log("data in getLike write profile page >> ", JSON.stringify(data));
    let parameter = {
      key: data.syskey,
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad WriterprofilePage');
  }
}
