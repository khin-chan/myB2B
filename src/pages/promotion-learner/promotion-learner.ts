...
/**
 * Generated class for the PromotionLearnerPage page.
 *
...
@Component({
  selector: 'page-promotion-learner',
  templateUrl: 'promotion-learner.html',
  providers: [FunctProvider,ChangelanguageProvider,ChangefontProvider]
})
export class PromotionLearnerPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public storage:Storage,public cdata: DatacleanComponent,
    ...
    console.log("Promotion Learner pass data == "+JSON.stringify(this.passData));
   // console.log("Promotion Learner pass type == "+JSON.stringify(this.passType));
     this.status = this.navParams.get("status");
    // console.log("Promotion Learner pass status == "+JSON.stringify(this.status));
    ...
    this.storage.get('b2bregData').then((data) => {
          ...
          /*else if(this.registerData == true){
            this._zone.run(() => {
              ...
              console.log("Promotion Learner Data");
          }
          ...
          /*this.storage.get('billStatus').then((data) => {
           ...
           console.log("Promotion Learner  get register data == "+JSON.stringify(this.registerData));
    });  
  }
...
ionViewDidEnter() {
  console.log('ionViewDidLoad PromotionLearnerPage');
  this.backButtonExit();
}
...
updateNotiData(){
  this.sqlite.create({
     ...
     this.db = db;
     this.db.executeSql("UPDATE myNoti SET noticountstatus=0 WHERE id= "+ this.passData.id,[]).then((data) => {
...
dismiss(){    
  this.storage.get('b2bregData').then((data) => {
    console.log("dismiss ..............");
          console.log(" Promotion Learner registerData = "+JSON.stringify(data));
          //if(this.status == 1){
            if(data == null || data == "" ){                
              this._zone.run(() => {
                this.navCtrl.setRoot(FirstStepPage);
...
getLike(data){
  ...
  console.log("data in getLike promotion learner page >> ", JSON.stringify(data));
  let parameter = {
    key : data.syskey,
...
onPlayingVideo(event,index) {
 ...
 getError(error,status){
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad PromotiomlearnerPage');
    this.storage.get('b2bregData').then((data) => {
      this.registerData = data;
      // this.menuCount();
      this.isLoading = true;
