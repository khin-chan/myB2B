...
/**
 * Generated class for the InterviewTabsPage page.
 *
...
@Component({
  selector: 'page-interview-tabs',
  templateUrl: 'interview-tabs.html',
  providers: [ChangelanguageProvider]
})
export class InterviewTabsPage {

  tab1Root = InterviewPage ;
  tab2Root = VideoPage;
  tab3Root = BvPage;
  changeicon : any = 'md-add';
  textMyan : any = ["အင်တာဗျူး","ဗီဒီယို","Brand Voice"];     /*"စီးပွားရေး","ဗီဒီယို",*/
  textEng : any = ["Interview","Video","Brand Voice"];
  textData : any = [];
  font : any ;
  pageTitle: any;
  noticount:any = 0;
  ...
  constructor(private superTabsCtrl: SuperTabsController,public navCtrl: NavController,public toastCtrl:ToastController,
              public changeLanguage : ChangelanguageProvider,public storage:Storage,public sqlite:SQLite,
  ...
  menuCount(){
    ...
    this.http.post(this.funct.ipaddress2+'serviceAppHistory/saveType',parameter).map(res => res.json()).subscribe(result => {
      ...
      if(result.state){
        console.log("Interview success menuCount >>>>>>>>");
      }
      else if(!result.sessionState){
        this.cdata.sessionAlert();
      }
      else{
        console.log("Interview unsuccess menuCount >>>>>>>>");
      }
    },
        error => {
  ...
  updateNotiCount(){
    this.sqlite.create({
      ...
      db.executeSql("UPDATE notiCount SET count = 0 WHERE type = 'interview'",[]).then((data) => {
      console.log("update data successfully", data);
      this.events.publish('interviewCount', 0);
      }, (error) => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad InterviewTabsPage');
  }
