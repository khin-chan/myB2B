...
/**
 * Generated class for the LeaderTabsPage page.
 *
...
@Component({
  selector: 'page-leader-tabs',
  templateUrl: 'leader-tabs.html',
  providers: [ChangelanguageProvider]
})
export class LeaderTabsPage {

  tab1Root = LeadershipPage ;
  tab2Root = VideoPage;
  tab3Root = BvPage;
  changeicon : any = 'md-add';
  textMyan : any = ["ခေါင်းဆောင်မှု","ဗီဒီယို","Brand Voice"];
  textEng : any = ["Leadership","Video","Brand Voice"];
  textData : any = [];
  font : any ;
  pageTitle: any;
  noticount:any = 0;
  ...
  menuCount(){
    ...
    this.http.post(this.funct.ipaddress2+'serviceAppHistory/saveType',parameter).map(res => res.json()).subscribe(result => {
      ...
      if(result.state){
        console.log("Leadership success menuCount >>>>>>>>");
      }
      else if(!result.sessionState){
        this.cdata.sessionAlert();
      }
      else{
        console.log("Leadership unsuccess menuCount >>>>>>>>");
      }
    },
        error => {
  ...
  updateNotiCount(){
    this.sqlite.create({
      ...
      db.executeSql("UPDATE notiCount SET count = 0 WHERE type = 'leadership'",[]).then((data) => {
      console.log("update data successfully", data);
      this.events.publish('leadershipCount', 0);
      }, (error) => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad LeaderTabsPage');
  }
