...
export class InterviewPage {
  ...
  font : any ;
  url:any = '';
  textMyan : any = ["အင်တာဗျူး","ကြိုက်တယ်","မှတ်ချက်","ဝေမျှမည်","ဒေတာ မရှိပါ။","ခု"];
  textEng : any = ["Interview", "Like","Comment","Share","No result found","Count"];
  textData : any = [];
  db:any;
  interviewData:any=[];
  interData:any=[];
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
              public platform:Platform,public logger: EventLoggerProvider,
              ...
              public events : Events,public sqlite:SQLite,public cdata:DatacleanComponent) {
                ...
                this.storage.get('billStatus').then((data) => {
                  ...
                  console.log("Interview BillStatus=" + JSON.stringify(this.billStatus));
                });
  ...
  getPinList(start,infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=interview'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.end == 10){
          ...
          for(let d = 0 ; d < this.pinmenuList.length ; d++){
            this.interData.push(this.pinmenuList[d]);
          } 
          //this.busData = this.pinmenuList;
             if(this.interData.length > 0){
              let inno ;
              if(this.interData.length>5)
                inno = 5;
              else if(this.interData.length<= 5)
                inno = this.interData.length;
                this.offline = true;
            ...
            console.log("busdata == "+this.interData.length);    
  ...
  getList(start, infiniteScroll ){
    ...
    console.log("response interview = ", JSON.stringify(parameter));
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=interview'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      console.log("response interview = ", JSON.stringify(data));
      if (data.data.length > 0){
        if(this.end == 10)
         this.menuList =[];
        this.nores = 1;
        ...
        if(this.insertData.length < 10){
          if(this.end == 10){         
            for(let d = 0 ; d < this.menuList.length ; d++){
              this.interData.push(this.menuList[d]);
            }                 
             if(this.interData.length > 0){
              let inno ;
              if(this.interData.length>5)
  ...
  insertData(inno){
    console.log("interview  data length = ", this.interData.length);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO interview(allData) VALUES (?)", [JSON.stringify(this.interData[i])]).then((data) => {
  ...
  deleteData(inno){
    this.db.executeSql("DELETE FROM interview", []).then((data) => {
  ...
  selectData(){
    ...
    this.pinmenuList = [];
    this.isLoading = true;
    this.db.executeSql("SELECT * FROM interview ", []).then((data) => {
      console.log("length == "+ data.rows.length);
      if(data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == "+ data.rows.item(i).allData);
          this.interviewData.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.interviewData));
        }
        this.menuList = this.interviewData;
        this.nores = 1;
        this.interviewData =[];
...
getBillValidate(){   //bill service      
  ...
  getLike(data,k,status){
    ...
    console.log("data in getLike interview page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
