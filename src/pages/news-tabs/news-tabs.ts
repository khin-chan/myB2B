...
/**
 * Generated class for the NewsTabsPage page.
 *
...
export class NewsTabsPage {
  ...
  textMyan : any = ["သတင်း","ဗီဒီယို","Brand Voice"];
  textEng : any = ["News","Video","Brand Voice"];
  textData : any = [];
  font : any ;
  pageTitle: any;
  noticount:any = 0;
  ...
  menuCount(){
    ...
    this.http.post(this.funct.ipaddress2+'serviceAppHistory/saveType',parameter).map(res => res.json()).subscribe(result => {
      ...
      if(result.state){
        console.log("News success menuCount >>>>>>>>");
      }
      else if(!result.sessionState){
        this.cdata.sessionAlert();
      }
      else{
        console.log("News unsuccess menuCount >>>>>>>>");
      }
    },
        error => {
  ...
  updateNotiCount(){
    this.sqlite.create({
      ...
      db.executeSql("UPDATE notiCount SET count = 0 WHERE type = 'news'",[]).then((data) => {
      console.log("update data successfully", data);
      this.events.publish('newsCount', 0);
      }, (error) => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad NewsTabsPage');
  }
