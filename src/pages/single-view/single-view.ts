...
export class SingleViewPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public storage:Storage,
              ...
              public http:Http,public toastCtrl:ToastController,public alert: AlertController,
              public changeLanguage : ChangelanguageProvider,public datepipe: DatePipe,public fba: FirebaseAnalytics,
              private _zone: NgZone,public clipboard:Clipboard) {
    ...
    console.log("single view type == "+JSON.stringify(this.passType));
     ...
     this.storage.get('billStatus').then((data) => {
      ...
      console.log("singleview billstatus == " + this.billStatus);
    });
    ...
    choosePayment() {
      ...
      this.navCtrl.push(PaymentPage, {
        notiPaymentStatus: this.notiPaymentStatus 
      });
    }
 ...
 //  this.url = "https://b2b101.app.link/R87NzKABHL";
  ...
  getSingleViewData(){
   this.loading = this.loadingCtrl.create({
       ...
       this.loading.present();      
     this.http.get(this.funct.ipaddress2 + 'serviceArticle/getArticleDataBySyskey?syskey='+this.passData+'&regsyskey='+this.registerData.syskey+'&sessionKey='+this.registerData.sessionKey).map(res => res.json()).subscribe(result => {
  ...
  getCommentData(){     //tmn
    this.http.get(this.funct.ipaddress2 + 'serviceQuestion/getCommentmobile?id='+this.passData+'&userSK='+this.registerData.syskey+'&sessionKey='+this.registerData.sessionKey).map(res => res.json()).subscribe(result => {
 ...
 backButtonExit(){
  ...
  getLike(data){
    ...
    console.log("data in getLike single view page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
