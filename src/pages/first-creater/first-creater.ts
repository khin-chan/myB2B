...
import { App, NavController, NavParams , Platform , LoadingController , ToastController , PopoverController , AlertController , Events} from 'ionic-angular';
...
/**
 * Generated class for the FirstCreaterPage page.
 *
...
@Component({
  selector: 'page-first-creater',
  templateUrl: 'first-creater.html',
  providers: [ChangelanguageProvider]
})
export class FirstCreaterPage {
  rootNavCtrl: NavController;
  font : any ;
  textMyan : any = ["စွန့်ဦးတီထွင်မှု","ကြိုက်တယ်","မှတ်ချက်","ဝေမျှမည်","ဒေတာ မရှိပါ။","ခု"];
  textEng : any = ["Innovation", "Like","Comment","Share","No result found","Count"];
  textData : any = [];
  nores :any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
              public platform:Platform,public changefont:ChangefontProvider,
              public storage:Storage,public datepipe: DatePipe,public logger: EventLoggerProvider,
    ...
    this.storage.get('billStatus').then((data) => {
      ...
      console.log("Innovation BillStatus=" + JSON.stringify(this.billStatus));     
    });
  ...
  getList(start, infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=innovation'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.innovationData.length <10){
          if(this.end == 10){          
            for(let d = 0 ; d < this.menuList.length ; d++){
              this.innovationData.push(this.menuList[d]);
            } 

          if(this.innovationData.length > 0){
            let inno ;
            if(this.innovationData.length>5)
              inno = 5;
            else if(this.innovationData.length<= 5)
              inno = this.innovationData.length;
              this.offline = false;
  ...
  getPinList(start,infiniteScroll){
    ...
    console.log("response innovation = ", JSON.stringify(parameter));
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=innovation'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      console.log("response innovation pin = ", JSON.stringify(data));
      if (data.data.length > 0){
        ...
        if(this.end == 10){
          ...
          for(let d = 0 ; d < this.pinmenuList.length ; d++){
            this.innovationData.push(this.pinmenuList[d]);
          } 
          //this.busData = this.pinmenuList;
            ...
            console.log("busdata == "+this.innovationData.length);                 
      }
      else{
        this.getList(this.start,infiniteScroll); 
  ...
  deleteData(inno){
    this.db.executeSql("DELETE FROM innovation", []).then((data) => {
  ...
  insertData(inno){
    console.log("news  data length = ", this.innovationData.length);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO innovation(allData) VALUES (?)", [JSON.stringify(this.innovationData[i])]).then((data) => {
  ...
  selectData(){
    ...
    this.pinmenuList =[];
    this.isLoading = true;
    this.db.executeSql("SELECT * FROM innovation ", []).then((data) => {
      console.log("length == "+ data.rows.length);
      if(data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == "+ data.rows.item(i).allData);
          this.innovation.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.innovation));
        }
        this.menuList = this.innovation;
        this.nores = 1;
        this.innovation =[];
  ...
  getLike(data,k,status){
    console.log("data in getLike first creater page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
...
getBillValidate(){   //bill service      
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad FirstCreaterPage');
  }
