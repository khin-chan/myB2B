...
import { App, NavController, NavParams, LoadingController, PopoverController, ViewController, Platform, ToastController, Events, AlertController } from 'ionic-angular';
...
export class NewsPage {
  ...
  font: any;
  url: any = '';
  textMyan: any = ["သတင်း", "ကြိုက်တယ်", "မှတ်ချက်", "ဝေမျှမည်", "ဒေတာ မရှိပါ။", "ခု"];
  textEng: any = ["News", "Like", "Comment", "Share", "No result found", "Count"];
  textData: any = [];
  db: any = [];
  newsData: any = [];
  news: any = [];
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
    ...
    this.storage.get('billStatus').then((data) => {
      ...
      console.log("New BillStatus=" + JSON.stringify(this.billStatus));
    });
    //--------------------
  ...
  getPinList(start, infiniteScroll) {
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK=' + this.registerData.syskey + '&mobile=' + this.registerData.t1 + '&firstRefresh=0&type=news' + '&sessionKey=' + this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0) {
        ...
        if (this.end == 10) {
          ...
          for (let d = 0; d < this.pinmenuList.length; d++) {
            this.newsData.push(this.pinmenuList[d]);
          }
          ...
          if (this.newsData.length > 0) {
            ...
            if (this.newsData.length > 5)
              inno = 5;
            else if (this.newsData.length <= 5)
              inno = this.newsData.length;
            this.offline = true;
        ...
        console.log("busdata == " + this.newsData.length);
  ...
  getList(start, infiniteScroll) {
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK=' + this.registerData.syskey + '&mobile=' + this.registerData.t1 + '&firstRefresh=0&type=news' + '&sessionKey=' + this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
        ...
        if (this.newsData.length < 10) {
          if (this.end == 10) {
            for (let d = 0; d < this.menuList.length; d++) {
              this.newsData.push(this.menuList[d]);
            }

            console.log("this is newsData == " + this.newsData.length);
  ...
  insertData(inno) {
    console.log("news  data length = ", this.newsData.length);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO news(allData) VALUES (?)", [JSON.stringify(this.newsData[i])]).then((data) => {
  ...
  deleteData(inno) {
    this.db.executeSql("DELETE FROM news", []).then((data) => {
  ...
  selectData() {
    ...
    this.pinmenuList = [];
    this.isLoading = true;
    this.db.executeSql("SELECT * FROM news ", []).then((data) => {
      console.log("length == " + data.rows.length);
      if (data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == " + data.rows.item(i).allData);
          this.news.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.news));
        }
        this.menuList = this.news;
        this.news = [];
        this.nores = 1;
  ...
  getLike(data, k, status) {
    ...
    console.log("data in getLike news page >> ", JSON.stringify(data));
    let parameter = {
      key: data.syskey,
