...
/**
 * Generated class for the BusinessTabsPage page.
 *
...
@Component({
  selector: 'page-business-tabs',
  templateUrl: 'business-tabs.html',
...
export class BusinessTabsPage {
  ...
  textMyan : any = ["စီးပွားရေး","ဗီဒီယို","Brand Voice"];     /*"စီးပွားရေး","ဗီဒီယို",*/
  textEng : any = ["Business","Video","Brand Voice"];
  textData : any = [];
  font : any ;
  pageTitle: any;
  noticount:any = 0;
  ...
  menuCount(){
    ...
    this.http.post(this.funct.ipaddress2+'serviceAppHistory/saveType',parameter).map(res => res.json()).subscribe(result => {
      ...
      if(result.state){
        console.log("business success menuCount >>>>>>>>");
      }
      else if(!result.sessionState){
        this.cdata.sessionAlert();
      }
      else{
        console.log("business unsuccess menuCount >>>>>>>>");
      }
    },
        error => {
  ...
  updateNotiCount(){
    this.sqlite.create({
      ...
      db.executeSql("UPDATE notiCount SET count = 0 WHERE type = 'business'",[]).then((data) => {
      console.log("update data successfully", data);
      this.events.publish('businessCount', 0);
      }, (error) => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad BusinessTabsPage');
  }
}
