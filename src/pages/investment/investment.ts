...
export class InvestmentPage {
  ...
  font : any ;
  textMyan : any = ["ရင်းနှီးမြှုပ်နှံမှု","ကြိုက်တယ်","မှတ်ချက်","ဝေမျှမည်","ဒေတာ မရှိပါ။","ခု"];
  textEng : any = ["Investment", "Like","Comment","Share","No result found","Count"];
  textData : any = [];
  nores :any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
    ...
    this.storage.get('billStatus').then((data) => {
      ...
      console.log("Investment BillStatus=" + JSON.stringify(this.billStatus));  
    });
  ...
  getPinList(start,infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=investment'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.end == 10){
          ...
          for(let d = 0 ; d < this.pinmenuList.length ; d++){
            this.investmentData.push(this.pinmenuList[d]);
          } 
          //this.busData = this.pinmenuList;
             if(this.investmentData.length > 0){
              let inno ;
              if(this.investmentData.length>5)
                inno = 5;
              else if(this.investmentData.length<= 5)
                inno = this.investmentData.length;
                this.offline = true;
            ...
            console.log("busdata == "+this.investmentData.length);    
  ...
  getList(start, infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=investment'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.investmentData.length < 10){
          if(this.end == 10){
            for(let d = 0 ; d < this.menuList.length ; d++){
              this.investmentData.push(this.menuList[d]);
            }   
             if(this.investmentData.length > 0){
              let inno ;
              if(this.investmentData.length>5)
  ...
  deleteData(inno){
    this.db.executeSql("DELETE FROM investment", []).then((data) => {
  ...
  insertData(inno){
    console.log("news  data length = ", this.investmentData.length);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO investment(allData) VALUES (?)", [JSON.stringify(this.investmentData[i])]).then((data) => {
  ...
  selectData(){
    ...
    this.pinmenuList =[];
    this.isLoading = true;    
    this.db.executeSql("SELECT * FROM investment ", []).then((data) => {
      console.log("length == "+ data.rows.length);
      if(data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == "+ data.rows.item(i).allData);
          this.investment.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.investment));
        }
        this.menuList = this.investment;
        this.nores = 1;        
        this.investment =[];        
  ...
  getLike(data,k,status){
    ...
    console.log("data in getLike investment page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
