...
export class LeadershipPage {
  ...
  font : any ;
  textMyan : any = ["ခေါင်းဆောင်မှု","ကြိုက်တယ်","မှတ်ချက်","ဝေမျှမည်","ဒေတာ မရှိပါ။","ခု"];
  textEng : any = ["Leadership", "Like","Comment","Share","No result found","Count"];
  textData : any = [];
  nores :any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
    ...
    this.storage.get('billStatus').then((data) => {
      ...
      console.log("Leadership BillStatus=" + JSON.stringify(this.billStatus));
    });
  ...
  getPinList(start,infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=leadership'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.end == 10){
          ...
          for(let d = 0 ; d < this.pinmenuList.length ; d++){
            this.leadershipData.push(this.pinmenuList[d]);
          } 
          //this.busData = this.pinmenuList;
             if(this.leadershipData.length > 0){
              let inno ;
              if(this.leadershipData.length>5)
                inno = 5;
              else if(this.leadershipData.length<= 5)
                inno = this.leadershipData.length;
                this.offline = true;
            ...
            console.log("busdata == "+this.leadershipData.length);    
  ...
  getList(start, infiniteScroll){
    ...
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=leadership'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      ...
      if (data.data.length > 0){
        ...
        if(this.leadershipData.length < 10){
          if(this.end == 10){
            for(let d = 0 ; d < this.menuList.length ; d++){
              this.leadershipData.push(this.menuList[d]);
            }
  ...
  deleteData(inno){
    this.db.executeSql("DELETE FROM leadership", []).then((data) => {
  ...
  insertData(inno){
    console.log("news  data length = ", this.leadershipData.length);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO leadership(allData) VALUES (?)", [JSON.stringify(this.leadershipData[i])]).then((data) => {
  ...
  selectData(){
    ...
    this.pinmenuList =[];
    this.isLoading = true;
    this.db.executeSql("SELECT * FROM leadership ", []).then((data) => {
      console.log("length == "+ data.rows.length);
      if(data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == "+ data.rows.item(i).allData);
          this.leadership.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.leadership));
        }
        this.menuList = this.leadership;
        this.leadership =[];
        this.nores = 1;
        this.isLoading = false;
  ...
  getLike(data,k,status){
    ...
    console.log("data in getLike leadership page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
