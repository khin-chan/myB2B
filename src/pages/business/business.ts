...
import { App, NavController, NavParams, LoadingController , PopoverController, ViewController, Platform,ToastController,AlertController,
...
export class BusinessPage {
  ...
  font : any ;
  url:any = '';
  textMyan : any = ["စီးပွားရေး","ကြိုက်တယ်","မှတ်ချက်","ဝေမျှမည်","ဒေတာ မရှိပါ။","ခု"];
  textEng : any = ["Business", "Like","Comment","Share","No result found","Count"];
  textData : any = [];
  db:any;
  businessData:any=[];
  busData:any=[];
  ...
  paymentData: any = [
   ...
   paymentData1: any = [
    { month: '၁', price: '၅၀', status: '1' },
    { month: '၄', price: '၁၀၀', status: '1' },
    { month: '၈', price: '၁၅၀', status: '1' },
    { year: '၁',price: '၂၀၀', status: '2' },
   ];

  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController,
              public platform:Platform,public storage:Storage,public loadingCtrl:LoadingController,
              public changefont:ChangefontProvider,public logger: EventLoggerProvider,
              public toastCtrl: ToastController,public datepipe: DatePipe,
              public http: Http,public alert: AlertController,
              public funct:FunctProvider,private appMinimize: AppMinimize,
    ...
    this.storage.get('billStatus').then((data) => {
      ...
      console.log("Business BillStatus=" + JSON.stringify(this.billStatus));
    });
  ...
  getPinList(start,infiniteScroll){
    ...
    console.log("response business = ", JSON.stringify(parameter));
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleListPin?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=business'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      console.log("response business pin = ", JSON.stringify(data));
      if (data.data.length > 0){
        ...
        if(this.end == 10){
          ...
          this.busData = [];
          for(let b = 0 ; b < this.pinmenuList.length ; b++){
            this.busData.push(this.pinmenuList[b]);
          } 
          //console.log("business busData for pinmenu == "+JSON.stringify(this.busData));
             if(this.busData.length > 0){
               this.offline = true;
              let inno ;
              if(this.busData.length>5)
                inno = 5;
              else if(this.busData.length<= 5)
                inno = this.busData.length;
                console.log("busdata == "+this.busData.length);  
                console.log("inno length for pinmenu == "+inno);
  ...
  getList(start, infiniteScroll ){
    ...
    console.log("response business = ", JSON.stringify(parameter));
    this.http.post(this.funct.ipaddress2 + 'serviceArticle/searchArticleList?searchVal=&userSK='+this.registerData.syskey+'&mobile='+this.registerData.t1+'&firstRefresh=0&type=business'+'&sessionKey='+this.registerData.sessionKey, parameter).map(res => res.json()).subscribe(data => {
      console.log("response business = ", JSON.stringify(data));      
      if (data.data.length > 0){
        if(this.end == 10)
         this.menuList =[];
        this.nores = 1;
        ...
        if(this.busData.length < 10){
          console.log("end end == "+this.end);          
          if(this.end == 10){
            if(this.busData.length < 10){
              for(let d = 0 ; d < this.menuList.length ; d++){
                this.busData.push(this.menuList[d]);
              }           
               if(this.busData.length > 0){
                let inno ;
                if(this.busData.length > 5)
                  ...
                  inno = this.busData.length;
                  console.log("inno data for menu == "+inno);
                  console.log("busData == "+this.busData.length);
                  this.offline = false;
                  this.deleteData(inno);
              }
          ...
          console.log("this is menu database length == "+this.pinmenuList.length);
          console.log("end end == "+this.end);
          console.log("busData for menu == "+this.busData.length);
          }
        }     
        if(infiniteScroll !=''){
  ...
  insertData(inno){
    console.log("business  data length = ", this.busData.length);
    console.log("inno length == "+inno);
    for (let i = 0; i < inno; i++) {
      this.db.executeSql("INSERT INTO business(allData) VALUES (?)", [JSON.stringify(this.busData[i])]).then((data) => {
  ...
  deleteData(inno){
    this.db.executeSql("DELETE FROM business", []).then((data) => {
  ...
  selectData(){
    ...
    this.pinmenuList =[];
    this.isLoading = true;
    this.db.executeSql("SELECT * FROM business ", []).then((data) => {
      console.log("length == "+ data.rows.length);
      if(data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          console.log("data.rows.item(i).allData == "+ data.rows.item(i).allData);
          this.businessData.push(JSON.parse(data.rows.item(i).allData));
          console.log("data rows:::" + JSON.stringify(this.businessData));
        }
        this.menuList = this.businessData;
        this.nores = 1;
        this.businessData =[];
  ...
  checkBill(){
    ...
    this.loading.present();
      ...
      if(this.cutphno == '7'){
        ...
        billip = "ooredoo";
        this.chekbillAlert =this.alert.create({
          subTitle: 'If you want to test with bill paymentin, you must register with MPT Phno.',
          cssClass:this.font,
          buttons: [
            {
              text: 'Ok',
...
getBillValidate(){   //bill service
  ...
  getLike(data,k,status){
    console.log("data in getLike business page >> ", JSON.stringify(data));
    let parameter = {
      key : data.syskey,
